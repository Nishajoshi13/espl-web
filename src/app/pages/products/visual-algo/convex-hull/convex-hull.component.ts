...
export class ConvexHullComponent implements OnInit, AfterViewInit {
  ...
  public bubbleChartColor = [  'rgba(75, 192, 192, 0.2)',
    'rgba(54, 162, 235, 0.2)',
    'rgba(255, 206, 86, 0.2)',
    'rgba(255, 99, 132, 0.2)',
  ...
  generateConvexHull(isInitialize: boolean = false) {
    ...
    if (isInitialize) {
      ...
      // @ts-ignore
      this.bubbleChartData[0].data.forEach((element: { x: number; y: number; }, index: number) => {
  ...
  rand(min: number | undefined, max: number | undefined) {
    if (this._seed == null) {
      this._seed = Date.now();
    }
    var seed = this._seed;
